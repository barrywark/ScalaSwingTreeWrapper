...
sealed trait TreeEditors extends EditableCellsCompanion {
    ...
    abstract class Editor[A] extends CellEditor[A] {
        ...
        protected[tree] def getTreeWrapper(peerTree: JTree) = peerTree match {
...
class Tree[A](private var treeDataModel: TreeModel[A] = TreeModel.empty[A])
  ...
  with Scrollable.Wrapper { thisTree =>
    ...
    val expansion = new TreeWillExpandListener with Publisher {
        override def treeWillExpand(e: jse.TreeExpansionEvent) {
            publish(TreeWillExpand[A](Tree.this, e.getPath.toIndexedSeq))
        }

        override def treeWillCollapse(e: jse.TreeExpansionEvent) {
            publish(TreeWillCollapse[A](Tree.this, e.getPath.toIndexedSeq))
        }

        peer.addTreeWillExpandListener(this)
    }
